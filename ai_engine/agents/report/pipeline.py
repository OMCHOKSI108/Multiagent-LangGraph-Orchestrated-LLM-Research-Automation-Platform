...
class ReportPipeline:
    ...
    def generate_report(
        ...
        # Stage 4: Convert to LaTeX (pass findings for real references)
        latex_source = self._convert_to_latex(markdown_report, task, template, findings=findings, job_id=job_id)
        ...
        try:
            ...
            print(f"[ReportPipeline] Word document saved: {docx_path}")
    ...
    def _convert_to_latex(
        ...
        findings: dict = None, job_id: str = "unknown"
    ) -> str:
        """Convert markdown report to IEEE-format LaTeX source."""
        safe_title = self._escape_latex(task)

        # Build real references if findings are available
        ref_data = {"bibitem_block": "", "count": 0}
        if findings:
            try:
                ref_data = build_references(findings)
            except Exception as ref_err:
                print(f"[ReportPipeline] Reference build warning: {ref_err}")

        # IEEE-standard preamble
        preamble = f"""\\documentclass[conference]{{IEEEtran}}
...
\\author{{\n  \\IEEEauthorblockN{{AI Research Engine}}
  \\IEEEauthorblockA{{\\textit{{Multi-Agent Research Platform}}}}\n}}
...
"""
        ...
        # Append real bibliography if available
        bib_section = ""
        if ref_data["bibitem_block"]:
            bib_section = (
                f"\n\\begin{{thebibliography}}{{00}}\n"
                f"{ref_data['bibitem_block']}\n"
