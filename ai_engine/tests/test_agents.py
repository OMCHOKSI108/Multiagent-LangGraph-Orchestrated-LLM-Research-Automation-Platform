...
class TestBaseAgent:
    ...
    def setup_method(self):
        ...
        self.test_agent = BaseAgent(
            name="TestAgent", system_prompt="Test system prompt", model_name="phi3:mini"
        )
...
class TestOrchestratorAgent:
    ...
    def test_paper_analysis_routing(self):
        ...
        mock_response.content = (
            '{"next_step": "paper_analysis", "plan": "Analyze the specific paper"}'
        )
        self.mock_llm.invoke.return_value = mock_response
    ...
    def test_literature_review_routing(self):
        ...
        mock_response = Mock()
        mock_response.content = (
            '{"next_step": "literature_review", "plan": "Survey the field"}'
        )
        self.mock_llm.invoke.return_value = mock_response

        state = {"task": "Survey recent advances in NLP", "paper_url": None}
    ...
    def test_fallback_routing(self):
        ...
        state_without_paper = {"task": "Test task", "paper_url": None}
...
class TestAgentRegistry:
    ...
    def test_registry_completeness(self):
        ...
        for agent_name in expected_agents:
            ...
            ), f"Agent {agent_name} is not a BaseAgent instance"
    ...
    def test_agent_initialization(self):
        ...
        for agent_name, agent_instance in AGENTS.items():
            ...
            assert hasattr(
                agent_instance, "run"
            ), f"Agent {agent_name} missing run method"
    ...
    def test_agent_run_interface(self):
        ...
        for agent_name in test_agents:
            ...
            try:
                ...
                assert isinstance(
                    result, dict
                ), f"Agent {agent_name} run() should return dict"
            finally:
                # Restore original LLM
                agent.llm = original_llm

    def test_lazy_registry_does_not_clobber_resolved_agent_llm_or_prompt(self):
        """Regression: _LazyAgent placeholders must not override real agent attrs."""
        from unittest.mock import patch
        from agents.registry import _LazyAgent

        class _DummyProvider:
            provider_name = "dummy"
            model_name = "dummy-model"

            def invoke_with_retry(self, messages: list):
                raise RuntimeError("dummy provider")

            async def ainvoke_with_retry(self, messages: list):
                raise RuntimeError("dummy provider")
        ...
        # Patch where BaseAgent imports the factory function.
        # When running tests from inside `ai_engine/`, modules are typically
        # imported as `agents.*` (not `ai_engine.agents.*`).
        with patch(
            "agents.base.get_llm_provider", return_value=_DummyProvider()
        ), patch(
            "ai_engine.agents.base.get_llm_provider",
            return_value=_DummyProvider(),
            create=True,
        ):
            # Use a fresh proxy to avoid shared global state from other tests.
            lazy_agent = _LazyAgent(".orchestrator.orchestrator", "OrchestratorAgent")
            inst = lazy_agent._resolve()

            assert inst.llm is not None
            assert getattr(inst, "system_prompt", "")
