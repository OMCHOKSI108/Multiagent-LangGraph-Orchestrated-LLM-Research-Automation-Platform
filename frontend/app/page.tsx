    ...
    <div className="relative isolate min-h-screen">
      ...
      <section className="section-shell pt-20 pb-16 lg:pb-24 flex flex-col lg:flex-row items-center gap-12 lg:gap-16 relative">
        ...
        <div className="flex-1 max-w-xl relative z-10">
          ...
          <div className="mb-8">
            <span className="relative inline-flex h-28 w-28 overflow-hidden rounded-full glow-teal">
              <div className="absolute inset-0 bg-gradient-to-br from-[#00F5D4]/20 to-[#3B82F6]/20 rounded-full" />
              <Image
                src={Logo}
                alt="MARP logo"
                fill
          ...
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#00F5D4]/20 bg-[#00F5D4]/5 mb-6">
            <span className="h-2 w-2 rounded-full bg-[#00F5D4] animate-pulse" />
            <span className="text-xs font-medium text-[#00F5D4] tracking-wide uppercase">
          ...
          {/* Description */}
          <p className="text-base md:text-lg leading-relaxed mb-8 max-w-lg" style={{ color: '#9CA3AF' }}>
        ...
        <div className="flex-1 w-full max-w-md lg:max-w-lg relative z-10">
          <div className="surface-card p-4 md:p-5 relative overflow-hidden">
            ...
            {/* Graph container */}
            <div className="relative h-[280px] md:h-[340px] rounded-2xl overflow-hidden border border-[rgba(0,245,212,0.2)] bg-[#0B0F1A]/80">
              <KnowledgeGraph3D />
            </div>
            
            {/* Info cards below graph */}
            <div className="relative mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(17,24,39,0.6)]">
                <p className="font-semibold mb-2" style={{ color: '#F9FAFB' }}>Live agent graph</p>
                <p className="text-sm" style={{ color: '#9CA3AF' }}>
                  Watch orchestration flow through topic discovery, literature review, gap synthesis, verification, and reporting.
                </p>
              </div>
              <div className="p-4 rounded-xl border border-[rgba(0,245,212,0.2)] bg-[rgba(0,245,212,0.05)] font-mono">
                <p className="text-[#00F5D4] mb-2">/deep climate risk</p>
                <p className="text-sm" style={{ color: '#9CA3AF' }}>
      ...
      <section className="section-shell pb-20 relative z-10">
        <div className="flex items-end justify-between gap-4 mb-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] mb-3" style={{ color: '#00F5D4' }}>
              What MARP orchestrates
            </p>
            <h2 className="text-2xl md:text-4xl font-bold mb-3">From question to structured output</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f, i) => (
            <div
              ...
              style={{
                background: i % 3 === 0 
                  ? 'linear-gradient(135deg, rgba(0,245,212,0.03) 0%, rgba(17,24,39,0.85) 100%)' 
                  : i % 3 === 1 
                  ? 'linear-gradient(135deg, rgba(59,130,246,0.03) 0%, rgba(17,24,39,0.85) 100%)'
                  : 'linear-gradient(135deg, rgba(139,92,246,0.03) 0%, rgba(17,24,39,0.85) 100%)'
              }}
            >
              <div className="mb-4">
                <div className="text-3xl mb-4">{f.icon}</div>
                <h3 className="text-lg font-semibold mb-2" style={{ color: '#F9FAFB' }}>{f.label}</h3>
      ...
      <section className="section-shell pb-24 relative z-10">
        <div className="surface-card p-6 md:p-8 relative overflow-hidden">
          {/* Background gradient */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#00F5D4]/5 via-transparent to-[#3B82F6]/5" />
          
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] mb-2" style={{ color: '#00F5D4' }}>
                Command palette
              </p>
              <h3 className="text-xl md:text-2xl font-bold mb-2" style={{ color: '#F9FAFB' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {COMMANDS.map(([cmd, desc]) => (
              <div
                key={cmd}
                className="flex gap-4 items-start p-4 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(17,24,39,0.5)] hover:border-[rgba(0,245,212,0.3)] hover:bg-[rgba(0,245,212,0.03)] transition-all duration-300"
              >
                <span 
                  className="text-sm font-mono font-semibold min-w-[130px] text-[#00F5D4]"
                >
                  {cmd}
                </span>
                <span className="text-sm" style={{ color: '#9CA3AF' }}>{desc}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer 
        className="border-t py-6 text-center relative z-10" 
        style={{ borderColor: 'rgba(255,255,255,0.06)', backgroundColor: 'rgba(11,15,26,0.8)' }}
      >
